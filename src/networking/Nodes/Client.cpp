...
string request(struct Client *client, string server_ip, string request, unsigned long size)
{
    ...
    // Make the connection.
    inet_pton(client->domain, server_ip.c_str(), &server_address.sin_addr);
    connect(client->socket, (struct sockaddr*)&server_address, sizeof(server_address));
    // Send the request;
    send(client->socket, request.c_str(), size, 0);
    // Read the response.
